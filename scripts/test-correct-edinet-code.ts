#!/usr/bin/env node
/**
 * 正しいedinetCodeでマッチング検証
 */

async function testCorrectEdinetCode() {
  ...
  // 正しいedinetCode
  const testCases = [
    { code: '29810', name: 'ランディックス', edinetCode: 'E35303' },
    { code: '34770', name: 'フォーライフ', edinetCode: 'E32793' },
  ];
  ...
  for (const tc of testCases) {
    ...
    // 最近180日をテスト（5日おき）
    for (let daysBack = 0; daysBack <= 180; daysBack += 5) {
...
testCorrectEdinetCode().catch(console.error);
